...
void rst::rasterizer::rasterize_triangle(const Triangle& t, const std::array<Eigen::Vector3f, 3>& view_pos)
{
    ...
    for (int x = left; x < right; ++x) {
        for (int y = bottom; y < top; ++y) {
            if(insideTriangle(x,y,t.v)==1) {
                auto [alpha, beta, gamma] = computeBarycentric2D(x, y, t.v);
                float Z = 1.0 / (alpha / v[0].w() + beta / v[1].w() + gamma / v[2].w());
                float zp = alpha * v[0].z() / v[0].w() + beta * v[1].z() / v[1].w() + gamma * v[2].z() / v[2].w();
                zp *= Z;
                int index = get_index(x, y);
                //if (zp<buffer[x-left][y-bottom]){
                if (zp < depth_buf[index]) {
                    //buffer[x-left][y-bottom]=zp;
                    depth_buf[index] = zp;
    ...
    //    * v[i].w() is the vertex view space depth value z.      v[i].w()是顶点视图空间深度值 z。
    //    * Z is interpolated view space depth for the current pixel                Z是当前像素的插值视图空间深度
    //    * zp is depth between zNear and zFar, used for z-buffer    zp 是介于 zNear 和 zFar 之间的深度，用于 z-buffer
